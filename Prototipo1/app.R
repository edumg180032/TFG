...
source("D:/UNIVERSIDAD/TFG/Codigo/Simmer.R")
...
data <- list(arrivals = arrivals, recursos = recursos, atributos = atributos, facturas = facturas, info_adicional = info_adicional, tiempos_espera = tiempos_espera, costos_promedios = costos_promedios)

# UI
ui <- fluidPage(
  titlePanel("TALLER de MANTENIMIENTO de VEHÍCULOS"),
    ...
    sidebarPanel(
      ...
      selectInput("tipoGrafico", "Selecciona el Tipo de Gráfico Específico:", 
                  choices = c("Circular", "Llegadas", "Densidad de Servicio", "Utilización de Recursos", 
                              "Tiempo de Espera", "Tiempo de Servicio", "Gasto por Cliente", 
  ...
  output$plot <- renderPlot({
    ...
    if (input$tipoGrafico == "Llegadas") {
      ...
      ggplot(filtered_data$arrivals %>% mutate(waiting_time = end_time - start_time - activity_time), aes(x = name, y = waiting_time)) +
        geom_boxplot() +
        labs(title = "Tiempo de Espera", x = "Entidad", y = "Tiempo de Espera (minutos)")
    } else if (input$tipoGrafico == "Tiempo de Servicio") {
      ggplot(filtered_data$arrivals, aes(x = name, y = activity_time)) +
        geom_boxplot() +
        labs(title = "Tiempo de Servicio", x = "Entidad", y = "Tiempo en el Sistema (minutos)")
    }
